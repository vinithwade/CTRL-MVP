...
export class ModelSyncEngine {
  ...
  syncFromDesign(changeType: 'create' | 'update' | 'delete', data: any): void {
    const event: SyncEvent = {
      ...
      type: `component.${changeType}` as SyncEventType,
      data,
  ...
  syncFromLogic(changeType: 'create' | 'update' | 'delete', nodeType: 'node' | 'connection', data: any): void {
    const event: SyncEvent = {
      ...
      timestamp: new Date().toISOString(),
      userId: 'current-user', // TODO: Get from auth context
      type: `logic.${nodeType}.${changeType}` as SyncEventType,
      data,
  ...
  syncFromCode(changeType: 'create' | 'update' | 'delete', data: any): void {
    const event: SyncEvent = {
      ...
      timestamp: new Date().toISOString(),
      userId: 'current-user', // TODO: Get from auth context
      type: `code.file.${changeType}` as SyncEventType,
      data,
  ...
  private processSyncEvent(event: SyncEvent): void {
    if (this.pendingSync) return // Prevent infinite loops
  ...
  // Event handlers
  private handleComponentCreate(component: UIComponent): void {
    this.project.components.push(component)
    this.updateCodeFromComponent(component)
    this.createLogicNodeForComponent(component)
  }

  private handleComponentUpdate(data: { id: string; updates: Partial<UIComponent> }): void {
  ...
  private handleLogicNodeCreate(node: LogicNode): void {
    this.project.logicGraph.nodes.push(node)
    this.updateCodeFromLogicNode(node)
  }

  private handleLogicNodeUpdate(data: { id: string; updates: Partial<LogicNode> }): void {
  ...
  private handleLogicNodeDelete(nodeId: string): void {
    this.project.logicGraph.nodes = this.project.logicGraph.nodes.filter(n => n.id !== nodeId)
    this.project.logicGraph.connections = this.project.logicGraph.connections.filter(
      c => c.fromNodeId !== nodeId && c.toNodeId !== nodeId
    )
    this.removeCodeForLogicNode(nodeId)
  }

  private handleLogicConnectionCreate(connection: LogicConnection): void {
    this.project.logicGraph.connections.push(connection)
    this.updateCodeFromConnection(connection)
  }

  private handleLogicConnectionDelete(connectionId: string): void {
    this.project.logicGraph.connections = this.project.logicGraph.connections.filter(
      c => c.id !== connectionId
    )
  ...
  private handleCodeFileUpdate(data: { path: string; content: string }): void {
  ...
  private generateReactComponent(component: UIComponent): string {
...
${styles}`
  ...
  private generatePropsInterface(component: UIComponent): string {
...
${propEntries.join('\n')}
  ...
  private generateStyles(component: UIComponent): string {
    ...
    if (styles.border) cssRules.push(`border: ${styles.border}`)
    ...
    if (styles.customCSS) cssRules.push(styles.customCSS)
  ...
  private generateJSX(component: UIComponent): string {
  ...
  private generateClassName(component: UIComponent): string {
  ...
  private updateLogicNodeForComponent(component: UIComponent): void {
    // Update existing logic nodes when component changes
  }

  private removeLogicNodeForComponent(componentId: string): void {
    // Remove logic nodes associated with deleted component
    this.project.logicGraph.nodes = this.project.logicGraph.nodes.filter(
      node => node.data?.componentId !== componentId
    )
  }

  private removeCodeForComponent(componentId: string): void {
  ...
  private removeCodeForLogicNode(nodeId: string): void {
    // Remove or update code affected by deleted logic node
  }

  private removeCodeForConnection(connectionId: string): void {
    // Remove or update code affected by deleted connection
  }

  private parseCodeToUpdateModel(file: CodeFile): void {
  ...
  private parseReactComponent(file: CodeFile): void {
  ...
  private generateVueComponent(component: UIComponent): string {
...
/* Component styles */
  ...
  private generateAngularComponent(component: UIComponent): string {
...
export class ${component.name}Component {
  ...
  importProject(jsonData: string): void {
    try {
      const project = JSON.parse(jsonData) as CTRLProject
